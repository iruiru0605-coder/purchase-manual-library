...
アプリはPCで起動したまま使います。スマホから同じライブラリを閲覧できます。**ポート番号**に注意（アイコン起動＝5174、開発モード＝5173）。

* **アイコン起動で使っている場合（通常）**
  * 同じWi-Fi内：スマホで `http://（PCのIPアドレス）:5174` を開く。PCのIPはコマンドプロンプトの `ipconfig` →「IPv4 アドレス」。
  * 外出先から：[Tailscale](https://tailscale.com)（無料・プライベートVPN）をPCとスマホ両方に入れ、`http://（PCのTailscale IP）:5174` を開く（IPはPCで `tailscale ip -4` で確認）。
* **`npm run dev`（開発モード）の場合**
  * 同じWi-Fi内で、起動時に表示される `Network:` のURL（ポート5173）をスマホで開く。
